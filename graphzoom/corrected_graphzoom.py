...
def graph_fusion(laplacian, feature, num_neighs, mcr_dir, coarse, fusion_input_path, \
                 search_ratio, fusion_output_dir, mapping_path, dataset, cmg_fusion_strategy="simple"):
    """
    ORIGINAL GraphZoom graph_fusion with CMG integration options.
    
    Args:
        cmg_fusion_strategy: "simple" (consistent with others) or "lamg" (use LAMG-style coarsening)
    """
    ...
    if coarse == "simple":
        ...
        # CMG fusion strategy options
        if cmg_fusion_strategy == "simple":
            print("[CMG] Using simple coarsening fusion (consistent with simple method)")
            mapping = sim_coarse_fusion(laplacian)
        elif cmg_fusion_strategy == "lamg":
            print("[CMG] Using LAMG-style coarsening fusion (for comparison with original results)")
            os.system('./run_coarsening.sh {} {} {} f {}'.format(mcr_dir, \
                    fusion_input_path, search_ratio, fusion_output_dir))
            mapping = mtx2matrix(mapping_path)
        else:
            raise ValueError(f"Invalid CMG fusion strategy: {cmg_fusion_strategy}. Use 'simple' or 'lamg'")
    else:
    ...
    # Save fused graph for LAMG and optionally for CMG
    if coarse == "lamg" or (coarse == "cmg" and cmg_fusion_strategy == "lamg"):
...
def main():
    ...
    parser.add_argument("--cmg_fusion_strategy", type=str, default="simple", choices=["simple", "lamg"], \
            help="CMG fusion strategy: 'simple' (consistent) or 'lamg' (original inconsistent)")
...
######Graph Fusion######
    if args.fusion:
        ...
        # Pass CMG fusion strategy to graph_fusion
        laplacian = graph_fusion(laplacian, feature, args.num_neighs, args.mcr_dir, args.coarse,\
                fusion_input_path, args.search_ratio, reduce_results, mapping_path, dataset, args.cmg_fusion_strategy)
...
######Graph Reduction######
    ...
    if args.coarse == "simple":
        ...
        print(f"**********************[CMG] Using fusion strategy: {args.cmg_fusion_strategy}")
        print(f"**********************[CMG] Coarsening input path: {coarsen_input_path}")
